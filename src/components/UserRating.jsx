...
const UserRating = ({ userId, initialRating, initialAttendance = 0, initialGoals = 0, onRated }) => {
    ...
    const handleRatingChange = async (newRating) => {
        ...
        if (newRating === 1 && rating === 1) {
            ...
            try {
                const response = await fetch(`https://teamalpino-backend.onrender.com/user/${userId}/rating`, {
                    method: 'PUT',
                    headers: {
    ...
    const handleGoalsChange = async (newGoals) => {
        ...
        setGoals(newGoals)
        try {
            const response = await fetch(`https://teamalpino-backend.onrender.com/user/${userId}/goals`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const handleAttendanceChange = async (newAttendance) => {
        ...
        try {
            const response = await fetch(`https://teamalpino-backend.onrender.com/user/${userId}/attendance`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
